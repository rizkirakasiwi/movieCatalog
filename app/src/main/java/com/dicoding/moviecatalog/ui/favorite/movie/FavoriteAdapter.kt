...
class FavoriteAdapter: PagedListAdapter<FavoriteMovie, FavoriteAdapter.FavoriteViewHolder>(
    FavoriteDiffUtil()
){
    inner class FavoriteViewHolder(private val binding : FavoriteLayoutBinding):RecyclerView.ViewHolder(binding.root){
        ...
        fun bind(favoriteMovie:FavoriteMovie){
            ...
            binding.titleText.text = favoriteMovie.title
            binding.ratingBar.rating = favoriteMovie.vote_average.toFloat() / 2f
        }
    }

    class FavoriteDiffUtil : DiffUtil.ItemCallback<FavoriteMovie>(){
